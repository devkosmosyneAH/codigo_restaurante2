  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text('Conoce nuestro espacio', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
          const SizedBox(height: 14),
          LayoutBuilder(
            builder: (context, constraints) {
              final columns = constraints.maxWidth >= 900 ? 4 : constraints.maxWidth >= 600 ? 3 : 2;
              final size = (constraints.maxWidth - (columns - 1) * 10) / columns;
              return Wrap(
                spacing: 10,
                runSpacing: 10,
                children: images.map((image) => ClipRRect(
                  ...
                  child: Image.network(
                    ...
                    height: size * .78,
                    fit: BoxFit.cover,
                    errorBuilder: (_, __, ___) => Container(width: size, height: size * .78, color: Colors.black12, child: const Icon(Icons.broken_image)),
                  ),
                )).toList(),
              );
            },
